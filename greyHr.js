...
const signInIntoGreyHr = async (req, res) => {
    ...
    while (!successText && retryCount < 3) {
        try {
            ...
            await page.waitForSelector('#username', { timeout: 50000 }); // Wait until input with id 'username' appears
            ...
            await gtButton.waitForSelector('div.btn-container > gt-button', { timeout: 50000 }); // Wait until the custom element 'gt-button' with class name 'hydrate' and attribute 'shade' equal to 'primary' appears in the DOM
            ...
            await page.waitForSelector('.alert-success', { timeout: 50000 }); // Wait until the success alert appears
            console.log('Success alert appeared');
            await page.waitForSelector('.alert-success p', { timeout: 50000 }); // Wait until the paragraph inside the success alert appears
